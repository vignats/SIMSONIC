function[probability] = getMetricsROI(probaMap, reconstruction, parameters, plotROI)
...
function[ROI] = ExtractROI(probaMap, parameters, reconstruction, plotROI)
...
% the bone/tissu interface in the specularity probability map.
    ...
    if isfield(parameters.interface, 'periost') && parameters.interface.periost > 0
        probaMap = probaMap.Bone;
    end
    ROI = probaMap(LimInf : LimSup, :);
    
    if plotROI
        figure
        pcolor(reconstruction.Xmm, reconstruction.Zmm, probaMap) 
