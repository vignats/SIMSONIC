function [vz,vx,LS_sd_residuals,LS_sd_error_Vz,LS_sd_error_Vx]=compute_VFlow_temporal_smooth_residual_selectTxRxpairs(I,Q,transmitAngles,receiveAngles,spatial_period_image_TxRx0deg,PRF,Average_kernel_size_x,Average_kernel_size_z,kernel_size_temporal_smoothing,selected_iTx,selected_iRx)
...
if (kernel_size_temporal_smoothing==0) % temporal averaging on all frames, vz and vx are 2D arrays
    ...
    for z = 1:NZ
        for x = 1:NX
            ...
            for iTxRx=1:N_TxRx_pairs
                thetaM = transmitAngles(selected_iTx(iTxRx),z,x); 
                    if ~isnan(Umat(z,x,iTxRx))
                        phiN = receiveAngles(selected_iRx(iTxRx),z,x);
                        AMat(iTxRx,1) = 1*(cos(thetaM) + cos(phiN));
                        AMat(iTxRx,2) = 1*(sin(thetaM) + sin(phiN));
                    end
            end
            ...
            AMat        = AMat(idx_good,:)/(PRF*spatial_period_image_TxRx0deg/pi); % !!!!!!!! modified by GR, 18 Nov 2022
