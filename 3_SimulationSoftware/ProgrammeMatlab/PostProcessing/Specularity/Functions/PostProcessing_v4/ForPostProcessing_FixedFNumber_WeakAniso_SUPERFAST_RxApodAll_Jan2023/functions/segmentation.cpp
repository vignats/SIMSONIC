...
//     memcpy(Parabola->pRawSegmentation, Contour, NX * sizeof(int)); // (added by GR, 2 May 2022)
// 
// 	delete [] Contour;
// }
...
int *
createContour(double **Im, const int Nx, const int Nz, const int IndXMin, const int IndXMax)
{
    double A, *CostFun1 = new double [2 * Nz], *CostFun2 = CostFun1 + Nz;
    int **CostPath = newTable2D(Nz, IndXMax-IndXMin+1);
    int x, z, n;
    int *ZIndexPath = new int [IndXMax-IndXMin+1];
    // from left to right
    memset(CostFun1, 0, 2 * Nz * sizeof(double));
    
    for (x = 0; x < (IndXMax-IndXMin+1); x++)
    {
        for (z = 1; z < Nz - 1; z++)
        {
            maxInArray(CostFun1 + z - 1, 3, A, n);
            CostPath[z][x] = z + (n - 1);
            CostFun2[z]    = A + Im[z][IndXMin+x];
        }
        memcpy(CostFun1, CostFun2, Nz * sizeof(double));
    }
    ...
    maxInArray(CostFun2, Nz, A, n);
    ZIndexPath[IndXMax-IndXMin] = n;
    for (x = (IndXMax-IndXMin) - 1; x >= 0; x--)
    {
        ZIndexPath[x] = CostPath[ZIndexPath[x + 1]][x+1];
    }
    // Desalocate.
...
void
segmentation (Parabola_st *Parabola, double ** Im, const int NX, const int NZ, double *X, double *Z, const double SegXMin , const double SegXMax)
{
    double resolutionZ = Z[2] - Z[1];
    double resolutionX = X[2] - X[1];
        
    int IndXMin = int((SegXMin-X[0])/resolutionX);
    int IndXMax = int((SegXMax*1.001-X[0])/resolutionX); // *1.001 to round up the index
    
	int x, *Contour    = createContour(Im, NX, NZ, IndXMin ,IndXMax);
    ...
    M11 = (IndXMax-IndXMin+1);
    for (x = 0; x < (IndXMax-IndXMin+1); x++)
    {
        d    = x;
        dres = d * resolutionX + X[IndXMin];
        d2   = dres * dres;
        M12  = M12 + dres;
        M13  = M13 + d2;
        M23  = M23 + dres * d2;
    ...
    int *ContourALL = new int [NX];
    for (x = 0; x < NX; x++)
    {
        if ((x >= IndXMin) && (x <= IndXMax)) {
            ContourALL[x] = Contour[x-IndXMin]+1; // Matlab convention
        }
        else {
            ContourALL[x] = -1;
        }
    ...
    // Now we retrieve the Min and Max of the Parabola that
    // will define the beginning and end of the following image.
    int ParabMin = NZ-1, ParabMax = 0; // modified by GR, December 22, 2022
    for (x = 1; x < NX; x++)
    {
        if (ContourALL[x] < ParabMin && ContourALL[x] > 0) ParabMin = ContourALL[x]; // modified by GR, December 22, 2022
        if (ContourALL[x] > ParabMax && ContourALL[x] > 0) ParabMax = ContourALL[x]; // modified by GR, December 22, 2022
    }
    Parabola->IdxMin = ParabMin; Parabola->IdxMax = ParabMax;
        
    // Retrieve the raw (by pixel) segmentation before being defined as a Parabola.
    memcpy(Parabola->pRawSegmentation, ContourALL, NX * sizeof(int)); // (added by GR, 2 May 2022)
    
	delete [] Contour, ContourALL;
}
