...

{
    ...
    double **BrentRange = refineCriticalAngleTable(Data,
            GlobalSetup.AngleCriticalMin, GlobalSetup.AngleCriticalMax);
        ...
        for (j = Data->ZMin; j < Data->ZMax; j++)
        {
            ...
            for (r = 0; r < Data->NR; r++)
            {
                ...
                *pTimeR      = brent(Setup.AngleCriticalMin, 0, Setup.AngleCriticalMax,
                                travelTimeUntilTissue, &Setup, AccuracyBrent, &AngleMin, IterMaxBrent);
                *pAngleR     = AngleMin;
...

{
    ...
    double **BrentRange = refineCriticalAngleTable(
            Data, GlobalSetup.AngleCriticalMin, GlobalSetup.AngleCriticalMax);

    omp_set_num_threads(GlobalSetup.NCore);
    #pragma omp parallel for schedule(static) \
    shared(Data,Image) \
    private(i,j,r,s,AngleMin,ZParabola)
    for (i = 0; i < Data->NX; i++)
    {
        ExpSetup_st Setup;
        memcpy(&Setup, &GlobalSetup, sizeof(ExpSetup_st));
        ...
        for (j = Data->ZMin; j < Data->ZMax; j++)
        {
            ...
            {
                double *pTimeR  = Image->pTimeR  + i*Data->NR*Data->NZ + j*Data->NR;
                double *pAngleR = Image->pAngleR + i*Data->NR*Data->NZ + j*Data->NR;
                double *pRayLengthR = Image->pRayLengthR + i*Data->NR*Data->NZ + j*Data->NR;
                double *pTimeT  = Image->pTimeT  + i*Data->NS*Data->NZ + j*Data->NS;
                double *pAngleT = Image->pAngleT + i*Data->NS*Data->NZ + j*Data->NS;
                double *pRayLengthT = Image->pRayLengthT + i*Data->NS*Data->NZ + j*Data->NS;
                ...
                for (r = 0; r < Data->NR; r++)
                {
                    ...
                    Setup.ZStart = Data->pZR[r];
                    *pTimeR      = brent(Setup.AngleCriticalMin, 0, Setup.AngleCriticalMax,
                                    travelTimeUntilBone, &Setup, AccuracyBrent, &AngleMin, IterMaxBrent);
                    *pAngleR     = AngleMin;
                    *pRayLengthR = RayLengthUntilTissue(AngleMin, &Setup);
                    pTimeR++; pAngleR++; pRayLengthR++;
                }
                ...
                for (s = 0; s < Data->NS; s++)
                {
                    Setup.XStart = Data->pXS[s];
                    Setup.ZStart = Data->pZS[s];
                    *pTimeT      = brent(Setup.AngleCriticalMin, 0, Setup.AngleCriticalMax,
                                    travelTimeUntilBone, &Setup, AccuracyBrent, &AngleMin, IterMaxBrent);
                    *pAngleT     = AngleMin;
                    *pRayLengthT = RayLengthUntilTissue(AngleMin, &Setup);
                    pTimeT++; pAngleT++; pRayLengthT++;
                ...
                double AnglePrior, double (*functionToTest)(double, void *))
{
    ...
    if (AnglePrior==0.)
    {
        double Theta, dTheta, TravelTime, TTmin = 1.;
