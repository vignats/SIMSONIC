function[Map] = MakeGeometryInterface(grid, probe, medium, interface, simu_dir, print)
...
%   - simu_dir: Directory where the simulation output will be saved.
%   - print: Logical value indicating whether to print the simulation map and rugosity calculation.
%
% Output Arguments:
%   - Map: Binary map representing the bone/soft tissue interface.
    ...
    [f, ~] = rsgene1D(Nx, grid.width, interface.rms, interface.corr);

    for i = 1:Nx
        Map(1: to_px(f(i) + interface.depth), i) = 1;
    end 
