function SPECU_MODEL_MATLAB_BONE=get_pix_simplified_tissue_model_NEW(ix,iz,interface_parameters,geometry,XR,...
...
%     if all(transmit_time)
        ...
        for tx=1:NTX
            T = [XR(tx) 0];
            TOF_MATLAB_BONE(tx)=get_tof_specu_one_layers(P0,T,SP_PARABOLA,C_TISSUE);
            TOF_DIFF_BONE(tx) = 2*hypot(xp-T(1),zp)/C_TISSUE;
        end
